...
import org.axonframework.extensions.multitenancy.messaging.eventhandling.processing.MultiTenantPooledStreamingEventProcessorModule;
...
/**
 * A multi-tenant aware version of the message handler configurer that creates
 * {@link MultiTenantPooledStreamingEventProcessorModule} instead of standard pooled streaming processors
 * when multi-tenancy is enabled for a processor.
 * <p>
 * This configurer is registered by {@link MultiTenantMessageHandlerLookup} when multi-tenancy
 * autoconfiguration is active.
 *
 ...
 */
public class MultiTenantMessageHandlerConfigurer implements ConfigurationEnhancer, ApplicationContextAware {

    private final Type type;
    private final List<String> handlerBeansRefs;
    private ApplicationContext applicationContext;

    public MultiTenantMessageHandlerConfigurer(Type type, List<String> beanRefs) {
        this.type = type;
    ...
    private void configureEventHandlers(ComponentRegistry registry) {
        groupNamedBeanDefinitionsByPackage().forEach((packageName, beanDefs) -> {
            ...
            Function<Configuration, ModuleBuilder<? extends Module>> moduleBuilder = (configuration) -> {
                ...
                // Check if multi-tenancy should be enabled for this processor
                // First try Spring ApplicationContext, then Axon Configuration, then default to enabled
                MultiTenantEventProcessorPredicate multiTenantPredicate;
                try {
                    multiTenantPredicate = applicationContext.getBean(MultiTenantEventProcessorPredicate.class);
                } catch (Exception e) {
                    multiTenantPredicate = configuration.getOptionalComponent(
                            MultiTenantEventProcessorPredicate.class
                    ).orElse(MultiTenantEventProcessorPredicate.enableMultiTenancy());
                }

                boolean useMultiTenancy = multiTenantPredicate.test(processorName);

                return switch (settings.processorMode()) {
                    case POOLED -> {
                        var moduleSettings = (PooledEventProcessorSettings) settings;
                        if (useMultiTenancy) {
                            // Use multi-tenant processor
                            yield MultiTenantPooledStreamingEventProcessorModule
                            ...
                            // Use standard processor with Spring customizations
                            yield EventProcessorModule
                                    .pooledStreaming(processorName)
                                    .eventHandlingComponents(componentRegistration)
                                    .customized(MultiTenantSpringCustomizations.pooledStreamingCustomizations(
                                            packageName,
                                            moduleSettings
                    ...
                    case SUBSCRIBING -> {
                        ...
                        // Subscribing processors don't support multi-tenancy in the same way
                        yield EventProcessorModule
                                .subscribing(processorName)
                                .eventHandlingComponents(componentRegistration)
                                .customized(MultiTenantSpringCustomizations.subscribingCustomizations(
                                        packageName,
                                        moduleSettings
                                ).andThen(c -> c.unitOfWorkFactory(configuration.getComponent(UnitOfWorkFactory.class))));
                    }
